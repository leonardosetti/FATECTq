...
void modConsultaMotorista()
{
    ...
    while (fread(&dadoMotorista, sizeof(dadoMotorista), 1, datafile))
    {
        if (dadoMotorista.state == 0)
        {
            printf("\n              %s %4d | %15s | %1s | %2d | %11s | %10s | %s ", sym_id, dadoMotorista.id, dadoMotorista.nome, dadoMotorista.sexo, dadoMotorista.idade, dadoMotorista.cnh, dadoMotorista.validade, sym_check);
        }
    }
