...
SELECT
	T.CODSETOR,
	T.NOME_COLABORADOR AS "TERCEIRIZADI"
FROM
	TERCEIRIZADO T
WHERE
	T.CODSETOR IS NULL;
...
FROM
	TERCEIRIZADO T
LEFT JOIN SETOR S ON
	T.CODSETOR IN S.CODSETOR
ORDER BY
	T.NOME_COLABORADOR DESC
;
...
FROM
	TERCEIRIZADO T
INNER JOIN SETOR S ON
	T.CODSETOR = S.CODSETOR
INNER JOIN AREA A ON
	S.AREA_CODIGO = A.AREA_CODIGO
WHERE
	A.NOME_AREA = 'INVESTIMENTOS'
ORDER BY
	T.COD_TERCEIRO ASC;
...
SELECT
	V.CODIGO,
	V.DATA_DA_VENDA,
	V.VOLUME_VENDA * V.PRECO_VENDA AS TOTAL_DIA
FROM
	VENDAS V
WHERE
	V.CODIGO_CIDADE = 3;
